        ...
        for (int i = 0, j = -1; i < array.length; i++) {
            try {
                if (array[i] == forch) { // если искомое нашли еще раз
                    isLast = false; // флаг isLast = FALSE(поиск до последнего элемента)
                ...
                if (!isLast) {
                    // пересоздаю массив пока находит искомое число и
                    // когда флаг isLast = TRUE значит больше совпадений нет
